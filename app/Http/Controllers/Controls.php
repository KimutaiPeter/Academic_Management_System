...
class Controls extends Controller
{
    ...
    function signin_page(Request $request){
        if($request->session()->has('email')){
            if($request->session()->get('type')=='student'){
                $rentry=student_table::where('email',session()->get('email'))->first();
    ...
    function login_process(Request $request){
        if($request->session()->has('email')){
            ...
            $rentry=student_table::where('email',$request->email)->first();
            if($rentry!=null){
                $email=$request->email;
                ...
                if($user_password===$rentry->password){
                    ...
                    $request->session()->put('student_id',$rentry->id);
                    $request->session()->put('type','student');

                    //Check if the student is admited or still awaiting approval
                    if($rentry->status=='admited'){
                        //If admited send redirect to the student elearning page
                        $request->session()->put('status','admited');
                        return redirect('student_elearning_page');
                    }else{
                        $request->session()->put('status','restricted');
                        $docs=student_docs_table::where('student_id',session()->get('id'));
                        return view('student_landing_page',['rentry'=>$rentry,'docs'=>$docs]);
                    }
                }else{
    ...
    function regestration_process(Request $request){
        ...
        $request->session()->put('type','student');
        $request->session()->put('status',"restricted"); //Change this to applied for applications of students
